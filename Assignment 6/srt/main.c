...
// copy contents for multithreading
void * prepare_for_multithread(void* thread_num) {
  //taken from main and placed here
  Vec3 camera_pos;
...
main( int argc, char **argv )
{
  ...
  for(int i = 0; i < nthreads; i++) {
    t_id[i] = i;
    int result = pthread_create(&threads[i], NULL, prepare_for_multithread,&t_id[i]);
